...
class AdminController extends Controller
{
    ...
    public function dashboard()
    {
        ...
        foreach ($orders as $order) {
            $totalSellAmount += $order->total_price;
        }
        // dd($totalSellAmount);
        ...
        // User Info
        $widget['total_users']              =        count($customers);  // User::count();
        
        $widget['verified_users']           =        count($customers);  // User::where('status', 1)->count();
        
        $widget['all_orders']               =        count($orders);    // Order::where('payment_status', '!=', 0)->count();
        
        $widget['delivered_orders']         =        count($orders);  //Order::where('payment_status', '!=', 0)->where('status',3)->count();
       
        $recent_orders                      =        Order::where('payment_status', '!=', 0)->latest()->take(6)->get();

        $widget['total_product']            =        count($products); //Product::whereHas('brand')->whereHas('categories')->count();

        // $widget['last_seven_days']          = Deposit::where('status', 1)->where('created_at', '>=', Carbon::today()->subDays(7))->sum('amount');
        $widget['last_seven_days']         = count(array_filter($orders, function($item) {
                                                    return $item->created_at >= Carbon::today()->subDays(7);
                                                }));
        $widget['last_fifteen_days']      = count(array_filter($orders, function($item) {
                                                    return $item->created_at >= Carbon::today()->subDays(15);
                                                }));
        $widget['last_thirty_days']       = count(array_filter($orders, function($item) {
                                                    return $item->created_at >= Carbon::today()->subDays(30);
                                                }));
        ...
        $widget['total_deposit_amount']     =  $totalSellAmount; //Deposit::where('status',1)->sum('amount');
        ...
        $payment['total_deposit_amount'] =  $totalSellAmount; //Deposit::where('status',1)->sum('amount');
        $payment['total_deposit_charge'] =  $totalSellAmount; // Deposit::where('status',1)->sum('charge');
