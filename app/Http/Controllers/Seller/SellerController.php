...
class SellerController extends Controller
{
    ...
    public function home()
    {
        ...
        // dd($orders->orders);
          foreach ($orders->orders as $key => $value1) {
            foreach ($value1->line_items as $key => $value2) {
                ...
                {
                        $processing = ($value1->financial_status == 'processing') ? ++$processing : $processing;
                        $delivered = ($value1->financial_status == 'delivered')  ? ++$delivered : $delivered;
                        $cancelled = ($value1->financial_status == 'cancelled')  ? ++$cancelled : $cancelled;
                        $pending = ($value1->financial_status == 'pending')  ? ++$pending : $pending;
                        $dispatched = ($value1->financial_status == 'dispatched')  ? ++$dispatched : $dispatched;
                        foreach ($value1->line_items as $key => $value) {
                            $product_approved = ($value->fulfillment_status == 'approved')  ? ++$product_approved : $product_approved;
                            $product_pending = ($value->fulfillment_status == 'pending')  ? ++$product_pending : $product_pending;
                        }
        ...
        //   dd($value2);
        $order['pending']       = $pending;   //OrderDetail::pendingOrder()->where('seller_id',$seller->id)->count();
        $order['processing']    = $processing; //OrderDetail::processingOrder()->where('seller_id',$seller->id)->count();
        $order['dispatched']    = $dispatched; //OrderDetail::dispatchedOrder()->where('seller_id',$seller->id)->count();
        $order['delivered']     = $delivered;  //OrderDetail::deliveredOrder()->where('seller_id',$seller->id)->count();
        $order['cancelled']     = $cancelled;  //OrderDetail::cancelledOrder()->where('seller_id',$seller->id)->count();
        $order['cod']           = OrderDetail::cod()->where('seller_id',$seller->id)->count();

        $product['approved']    = $product_approved; //Product::active()->where('seller_id',$seller->id)->count();
        $product['pending']     = $product_pending;     //Product::pending()->where('seller_id',$seller->id)->count();
        $product['total_sold']  = Product::active()->where('seller_id',$seller->id)->sum('sold');
        
        function daysAmount($days,$orders)
        {
            $days_amount = 0;
            $filter_results =  array_filter($orders->orders, function($item) use($days) {
                return $item->created_at >= Carbon::today()->subDays($days);
            });
            foreach ($filter_results as $key => $value) {
                $days_amount = $days_amount + $value->total_price;
            }
            return $days_amount;
        }
        $sale['last_seven_days']   = daysAmount(7,$orders);   //SellLog::where('seller_id',$seller->id)->where('created_at', '>=', Carbon::today()->subDays(7))->sum('after_commission');

        // dd( $last_seven_days);
        $sale['last_fifteen_days']        = daysAmount(15,$orders);  //SellLog::where('seller_id',$seller->id)->where('created_at', '>=', Carbon::today()->subDays(15))->sum('after_commission');

        $sale['last_thirty_days']         = daysAmount(30,$orders);  //SellLog::where('seller_id',$seller->id)->where('created_at', '>=', Carbon::today()->subDays(30))->sum('after_commission');
        ...
        $withdraw['total'] = $total_amount;//Withdrawal::where('seller_id',$seller->id)->where('status',1)->sum('amount');
